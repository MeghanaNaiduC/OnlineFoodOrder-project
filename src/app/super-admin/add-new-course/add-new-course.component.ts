...
export class AddNewCourseComponent implements OnInit {
  ...
  constructor(public datepipe: DatePipe, public dashboardService : DashboardService,public dialogRef: MatDialogRef<AddNewCourseComponent>, @Inject(MAT_DIALOG_DATA) public course: any,private alertService: AlertService,public fb : FormBuilder) {
    if(this.course)
  ...
  createUserForm() {
    this.addUserForm = this.fb.group({
      ...
      addFitsrName : new FormControl('', [Validators.required]),
  ...
  get addFitsrName() {return this.addUserForm.get('addFitsrName'); }
  ...
  get province() {return this.addUserForm.get('province'); }
  
  get country() {return this.addUserForm.get('country'); }
  
  get city() {return this.addUserForm.get('city'); }
  
  get courseAddress() {return this.addUserForm.get('courseAddress'); }
  ...
  saveAndCreateCourse() {
    ...
    adminUser['name'] = this.addUserForm.value.addFitsrName + " " + this.addUserForm.value.lastName;
    adminUser['phone'] = this.addUserForm.value.addContact;
    adminUser['email'] = this.addUserForm.value.addEmail;
    adminUser['country'] = this.addUserForm.value.country;
    adminUser['state'] = this.addUserForm.value.province;
    adminUser['city'] = this.addUserForm.value.city;
    adminUser['address'] = this.addUserForm.value.courseAddress;
    adminUser['code'] = 91;
    payment['firstName'] = this.addUserForm.value.addFitsrName;
    payment['lastName'] = this.addUserForm.value.lastName;
    payment['email'] = this.addUserForm.value.addEmail;
    payment['mode'] = "Cheq";
    payment['pfToken'] = ""; 
    payment['amount'] = 1792.224;
    finalObj['fullPayment'] = 1;
    finalObj['selectedMonths'] = selectedMonths;
    finalObj['course'] = course;
    finalObj['adminUser'] = adminUser;
    finalObj['payment'] = payment;
    finalObj['selectedMonths'] = selectedMonths;
    console.log(finalObj);
    this.dashboardService.registerCourse(finalObj).then(data => {
      if(data.success) {
